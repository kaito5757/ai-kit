---
description: 厳密な検証ループを回しながら実装計画を実行する
...
Run /ecc-prp-plan <feature-description> to create a plan first.
```
...
> Next step: Run `/ecc-prp-pr` to create a pull request, or `/ecc-code-review` to review changes first.
```
...
- Run `/ecc-prp-commit` to commit with a descriptive message
- Run `/ecc-prp-pr` to create a pull request
- Run `/ecc-prp-plan <next-phase>` if the PRD has more phases
