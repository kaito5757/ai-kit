---
description: tdd-workflow スキルへの互換エントリポイント（レガシー）。通常はスキルを直接使うことを推奨。
---

# TDD Command (Legacy Shim)

Use this only if you still invoke `/ecc-tdd`. The maintained workflow lives in `skills/tdd-workflow/SKILL.md`.
...
- Use `/ecc-plan` first to understand what to build
- Use `/ecc-tdd` to implement with tests
- Use `/ecc-build-fix` if build errors occur
- Use `/ecc-code-review` to review implementation
- Use `/test-coverage` to verify coverage

## Related Agents

This command invokes the `ecc-tdd-guide` agent provided by ECC.

The related `tdd-workflow` skill is also bundled with ECC.

For manual installs, the source files live at:
- `agents/ecc-tdd-guide.md`
