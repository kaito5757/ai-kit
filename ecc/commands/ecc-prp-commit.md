---
description: "自然言語でファイル指定して素早くコミット — コミット内容を自然言語で説明すればよい"
...
Next steps:
  ...
  - /ecc-prp-pr            → create a pull request
...
|---|---|
| `/ecc-prp-commit` | Stages all, auto-generates message |
| `/ecc-prp-commit staged` | Commits only what's already staged |
| `/ecc-prp-commit *.ts` | Stages all TypeScript files, commits |
| `/ecc-prp-commit except tests` | Stages everything except test files |
| `/ecc-prp-commit the database migration` | Finds DB migration files from status, stages them |
| `/ecc-prp-commit only new files` | Stages untracked files only |
