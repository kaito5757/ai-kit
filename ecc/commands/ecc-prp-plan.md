---
description: コードベース解析とパターン抽出を行い、網羅的な機能実装計画を作成する
...
> Next step: Run `/ecc-prp-implement .claude/PRPs/plans/{name}.plan.md` to execute this plan.
```
...
- Run `/ecc-prp-implement <plan-path>` to execute this plan
- Run `/ecc-plan` for quick conversational planning without artifacts
- Run `/ecc-prp-prd` to create a PRD first if scope is unclear
````
